...
def show_ref(message):
    ...
    caption = (
        f"✅ **Do'stingizni taklif qilish orqali ham ma'lum bir miqdorda daromat oling!**💰\n\n"
        f"Do'stingizni botimizga taklif qiling, agar do'stingiz hayvon sotib olsa sizning hisobingizga 10% miqdorda bonus taqdim etiladi 💸\n\n"
        f"⚡️ **Boshlash uchun bosing:**\n{ref_link}"
    )
    ...
    markup.row(
        types.InlineKeyboardButton(
            '↗️ Ulashish',
            switch_inline_query=f"Do'stingizni botimizga taklif qiling: {ref_link}",
        )
    )
...
def deposit_start(message):
    text = (
        ...
        f"💳 **Karta:** `{CARD_NUMBER}`\n\n"
        f"👤 **Quyidagi kartaga to'lov qiling va to'lov chekini yuboring.**\n\n"
        f"▫️ 24000 so'm dan kamroq kiritilgan pullar tushurib berilmaydi."
    )
    markup = types.InlineKeyboardMarkup()
    markup.row(
        types.InlineKeyboardButton(
...
def pay_done_callback(call):
    ...
    text = (
        ...
        f"**Eslatma:**\n"
        f"▫️ Chekda sana, vaqt, ism familiya miqdor to'liq ko'rinishi kerak!\n"
        f"▫️ Soxta chek yasash yoki umuman to'lov qilmaslik, bloklanishga sabab bo'ladi."
    )
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
...
def view_animal(call):
    ...
    text = (
        ...
        f"✅ **Hayvon 30 kun davomida sizga foyda keltiradi. 30 kundan so'ng daromad berish vaqti tugaydi.**\n\n"
